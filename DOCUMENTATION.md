...
- **Palettes**: `:root` (light, the default) and `html.dark` (overrides) at
  the top of `css/site.css` — full table in recipe 1.3. The **landing page is
  exempt on purpose**: it's always black with the red skull, trig, teponaztli
  and date, in both themes. Only the overlay and the inner pages flip.
- **The toggle**: sun/moon button, injected by `js/site.js` on every page,
  ...
  **only while the overlay is open** (next to the teponaztli) and disappears
  when you return to the black scene. Light shows the moon (click → dark),
  dark shows the sun (click → light).
- **Persistence**: the choice lands in `localStorage` under `theme` and is
  re-applied on every page **before first paint** by a one-line `<script>` in
  each page's `<head>` (the only piece that isn't single-sourced — it must
  run before the CSS to avoid a flash; if you ever hand-make a new page,
  copy that line). The OS dark-mode preference is deliberately ignored.
